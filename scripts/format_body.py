...
PAGE_TOP_MM=37; PAGE_BOTTOM_MM=28; PAGE_LEFT_MM=28; PAGE_RIGHT_MM=26
LINE_SPACING_PT=28; SIZE_2=Pt(22); SIZE_3=Pt(16); SIZE_4=Pt(14); INDENT_CHARS=2
...
_h1=LEVEL_PATTERNS[0][0].lstrip('^'); _h2=LEVEL_PATTERNS[1][0].lstrip('^')
HEADING_SPLIT_RE=re.compile(r'^('+_h1+'|'+_h2+')')

FONT_CANDIDATES={
    '方正小标宋简体':['方正小标宋简体','FZXiaoBiaoSong-B05S','FZXiaoBiaoSong'],
    '黑体':['黑体','SimHei'],'楷体_GB2312':['楷体_GB2312','KaiTi_GB2312','KaiTi','楷体'],
    '仿宋_GB2312':['仿宋_GB2312','FangSong_GB2312','FangSong','仿宋'],
}
FONT_FALLBACK={'方正小标宋简体':'宋体','黑体':'SimHei','楷体_GB2312':'楷体','仿宋_GB2312':'仿宋'}

# 层级映射常量（预定义，避免运行时构建反向映射）
...
def warn(warnings,msg):
    warnings.append(msg); print(f"  ⚠ {msg}",file=sys.stderr)
...
def set_cn_font(run,font_name,size,bold=False):
    ...
    for k,v in [('w:eastAsia',font_name),('w:ascii','Times New Roman'),('w:hAnsi','Times New Roman'),('w:cs','Times New Roman')]:
        rFonts.set(qn(k),v)

def set_line_spacing_28(para):
    pPr=para._element.get_or_add_pPr(); spacing=pPr.find(qn('w:spacing'))
...
def add_spacer(doc,font_name,available_fonts,warnings,count=1):
    ...
    for _ in range(count):
        ...
        set_line_spacing_28(para); run=para.add_run(''); set_cn_font(run,af,SIZE_3,False)
...
def _add_page_text_run(para,text,font_name='宋体',size=SIZE_4):
    """添加页码文本 run，设置中文字体"""
    run=para.add_run(text); run.font.size=size
    rPr=run._element.get_or_add_rPr(); rFonts=OxmlElement('w:rFonts')
    rFonts.set(qn('w:eastAsia'),font_name); rPr.insert(0,rFonts)
    return run
...
def add_page_number(doc):
    ...
    it.text=' PAGE '; run._element.append(it)
...
def add_title(doc,text,available_fonts,warnings):
    ...
    run=para.add_run(text); set_cn_font(run,af,SIZE_2,False)
    ...
    for i,item in enumerate(body_items):
        ...
        if last_level_num>0 and curr_num>last_level_num+1:
            ...
            for lv in range(last_level_num+1,curr_num):
                key=LEVEL_NUM_TO_KEY[lv]; name,example=LEVEL_NAMES[key]
            ...
            issues.append({'index':i,'text':text[:60],'level':style['level'],'prev_level':prev_key,'skipped':[LEVEL_NUM_TO_NAME[lv] for lv in range(last_level_num+1,curr_num)],'suggestion':suggestion})
        last_level_num=curr_num
    return issues

STYLE_SALUTATION={'font_name':'仿宋_GB2312','size':SIZE_3,'bold':False,'is_heading':False,'level':'salutation'}

def add_body_paragraph(doc,text,style,available_fonts,warnings,alignment=None,no_indent=False):
    af=resolve_font(style['font_name'],available_fonts,warnings)
    bf=resolve_font('仿宋_GB2312',available_fonts,warnings)
    is_heading=style.get('is_heading',False); bold=style.get('bold',False); size=style.get('size',SIZE_3)
    para=doc.add_paragraph()
    ...
    if is_heading:
        ...
        if m:
            am=stripped[m.end():]; sm=re.search(r'[：。]',am)
            if sm and len(am[:sm.start()].strip())>=2: heading_split=m.end()+sm.end()
    if heading_split:
    ...
    else: run=para.add_run(stripped); set_cn_font(run,af,size,bold)
...
def detect_available_fonts(font_dir=None):
    ...
    elif system=='Darwin': font_paths=[Path('/Library/Fonts'),Path('/System/Library/Fonts'),Path.home()/'Library'/'Fonts']
    else: font_paths=[Path('/usr/share/fonts'),Path('/usr/local/share/fonts'),Path.home()/'.fonts']
...
def _render_salutation(doc,data,available_fonts,warnings,body_font):
    ...
    if sal: add_body_paragraph(doc,sal,STYLE_SALUTATION,available_fonts,warnings,no_indent=True)
...
def _render_body(doc,data,available_fonts,warnings,body_font):
    ...
    for item in data.get('正文',[]):
        if isinstance(item,dict) and 'image' in item:
            ip=item['image']
            if not os.path.exists(ip): warn(warnings,f"图片不存在: {ip}"); continue
            ...
            try: run.add_picture(ip,width=Mm(wm),height=Mm(hm))
            except Exception as e:
                warn(warnings,f"图片插入失败 ({os.path.basename(ip)}): {e}")
...
def _render_signature(doc,data,available_fonts,warnings,body_font):
    sig=data.get('署名','').strip(); dt=data.get('日期','').strip()
    if not (sig or dt): return
    add_spacer(doc,'仿宋_GB2312',available_fonts,warnings,count=2)
    if sig:
        para=doc.add_paragraph(); para.alignment=WD_ALIGN_PARAGRAPH.RIGHT
        para.paragraph_format.space_before=Pt(0); para.paragraph_format.space_after=Pt(0)
        set_line_spacing_28(para); para.paragraph_format.first_line_indent=Pt(0)
        run=para.add_run(sig); set_cn_font(run,body_font,SIZE_3,False)
    if dt:
        para=doc.add_paragraph(); para.alignment=WD_ALIGN_PARAGRAPH.RIGHT
        para.paragraph_format.space_before=Pt(0); para.paragraph_format.space_after=Pt(0)
        para.paragraph_format.right_indent=Pt(SIZE_3.pt*2)
        set_line_spacing_28(para); para.paragraph_format.first_line_indent=Pt(0)
        run=para.add_run(dt); set_cn_font(run,body_font,SIZE_3,False)
...
def main():
    ...
    parser.add_argument('--from-extract',action='store_true',help='输入为 extract_docx.py 原始输出（列表），自动提取标题和正文')
    ...
    # --from-extract: 智能提取标题（首项非层级文本→标题，否则生成占位标题）
    if args.from_extract:
        if isinstance(raw,list) and len(raw)>0:
            first=raw[0]
            if isinstance(first,str) and not any(re.match(p,first.strip()) for p,*_ in LEVEL_PATTERNS):
