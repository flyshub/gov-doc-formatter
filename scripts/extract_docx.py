...
def extract_with_numbering(filepath, image_dir=None):
    ...
    for child in body:
        ...
        if tag == 'p':
            ...
            texts = child.findall('.//{%s}t' % W_NS)
            text = ''.join(t.text or '' for t in texts).strip()
            numPr = child.find('{%s}pPr' % W_NS)
            number_prefix = ''
            if numPr is not None:
                numPr_elem = numPr.find('{%s}numPr' % W_NS)
                if numPr_elem is not None:
                    ilvl_elem = numPr_elem.find('{%s}ilvl' % W_NS)
                    numId_elem = numPr_elem.find('{%s}numId' % W_NS)
            ...
            full_text = (number_prefix + text).strip() if (number_prefix or text) else ''
            if images:
                if full_text:
