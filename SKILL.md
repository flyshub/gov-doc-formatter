...
- **`.docx`** → 使用 `extract_docx.py` 提取文本（**必须用此脚本**，解析 Word 自动编号并保留加粗格式）
  ```bash
...
```json
{
  ...
  "正文": [
    ...
    {"text": "完整段落", "runs": [{"text": "加粗部分", "bold": true}, {"text": "非加粗", "bold": false}]},
...
> `extract_docx.py` 自动检测原文加粗并输出 `runs` 格式，无需手动构造。
